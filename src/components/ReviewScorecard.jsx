...
const ITEMS = [
  ...
  { tier: 1, name: 'Signed, append-only audit log', status: 'partial', note: 'Now tamper-evident rather than merely a table: every event carries a sequence number, the previous row hash, a hash of its own facts, and an HMAC under a server-only secret. Editing a row, deleting one, or forging a replacement all become visible, and anyone can run the check. Still not append-only — the table remains writable, so the guarantee is detection, not prevention, and older rows written before chaining are unverifiable either way.' },
  ...
  { tier: 2, name: 'Cite-or-admit grounding (per-claim verification, review queue)', status: 'partial', note: 'Built. Each factual claim is extracted and checked against the retrieved context for a supporting quote; a response with unsupported claims is withheld from the caller and queued for human review instead of returned as verified. An uncited claim that happens to be true still fails, because the harness cannot tell knowledge from invention. Partial because the checker is itself a language model — the human queue is the real authority.' },
  { tier: 3, name: 'Token budget per identity (the actual exhaustion bound)', status: 'partial', note: 'Built. Spend is capped in estimated tokens per agent per window, reserved before each model call and reconciled after, with the harness\'s own judge and dedup calls charged to the same budget rather than hidden. Partial because the platform does not report provider usage, so every figure is a chars/4 estimate — enough to bound runaway spend, not to bill against.' },
  ...
  { tier: 3, name: 'Rate limiting per identity', status: 'yes', note: 'Built. Configurable requests-per-window per agent, enforced server-side at admission by reserving a slot and then ranking it, so concurrent requests are ordered rather than all reading the same stale count. A request cap alone never bounded exhaustion, which is why the token budget sits beside it.' },
  ...
  return (
    <div>
      ...
      <p className="text-xs text-slate-500 leading-relaxed mb-4">
        Every requirement from the review, with what is actually in this app. The one remaining Tier 1 gap —
        a real sandbox — is infrastructure below the app layer, so this is still enforcement without
        containment: a halt stops the output, not the action.
      </p>
